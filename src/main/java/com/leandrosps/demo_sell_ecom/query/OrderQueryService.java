...
public class OrderQueryService {
        ...
        public record GetOrderData(String orderId, String clientId, String clientName, String clientEmail,
        ...
        public GetOrderByIdOuput getOrder(String order_id, String userEmail) {

                var getOrderSql = "SELECT * FROM orders WHERE id = :id";
                ...
                return new GetOrderByIdOuput(orderData.getId(), orderData.getTotal(), orderData.getStatus(),
                                orderData.getClient_id(), orderData.getClient_email(), orderData.getCoupon(),
                                orderData.getCreated_at(), orderItems);
        }
        ...
        public List<GetOrdersOfAClientOutPut> getOrdersOfAClient(String client_id, String status) {
                ...
                orders.forEach(order -> {
                        List<OrderItemDbModel> orderItems = jdbcClient
                        ...
                        result.add(new GetOrdersOfAClientOutPut(order.orderId(), order.clientId(), order.clientName(),
                                        order.clientEmail(), order.orderTotal(), order.orderStatus(),
                                        order.orderCoupon(), order.orderCreatedAt(), orderItems));
                });
