...
public class OrderServiceTest {
  ...
  static {
    ...
    mysqldb.withDatabaseName("testdb").withUsername("testuser").withPassword("testpass").withInitScripts("schema.sql",
        "data.sql");
    mysqldb.start();
  }
  ...
  void shouldPlaceAnOrderAndConsult() {
    // English project
    // Add Cupom to the order
    // Add Payment Paypal fake Gateway moke apit: https://util.devi.tools/api/v2/authorize
  ...
  void shouldNotBeAbleToPlaceAnOrderWithInvalidProduct() {
    List<ItemInputs> itemsInput = new ArrayList<>();
    itemsInput.add(new ItemInputs("284791a5-5a40-4a31-a60c-d2df68997569", 1));
    this.orderService.placeOrder("joao@exemplo.com.br", itemsInput, "");

    var output = orderQueryService.getOrdersOfAClient(client_id_db, "WAITING_PAYMENT");
    assertThat(output.size()).isLessThan(2);
  }
}
